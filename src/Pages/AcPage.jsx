...
const [selected,setselected]=useState([]);
    ...
    if(selected.length==0){
      ...
      data=acData.filter((apple)=>selected.includes(apple.company));
    }

    let uniquecompany=[...new Set(acData.map((mango)=>mango.company))]
    ...
    <div className="prosection2">
        {
            data.map((item)=>{
                return <div className="imagebox2" key={item.id}>
                  <Link to={`/ac/${item.id}`}><img src={item.image} alt="" className="proimage2"/>
                  </Link>
