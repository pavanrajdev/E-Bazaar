...
const SpeakersPage = () => {
 const [selected,setselected]=useState([]);
    ...
    if(selected.length==0){
      ...
      data=speakerData.filter((apple)=>selected.includes(apple.brand));
    }

    let uniquebrand=[...new Set(speakerData.map((mango)=>mango.brand))]
      ...
      <div style={{width:"150px", fontWeight:"bold", padding:"3px", fontSize:"large", backgroundColor:"rgb(247, 247, 247)"}}>
{
  uniquebrand.map((orange)=>{
    return <label key={orange} style={{width:"200px"}}>
      <input type="checkbox"
